...
//printf("gmavi-3\n");
  ...
  is_align = length % 2;
//#warning " 333333333333333222222222221111111111 "
//write extra data first (this lets index search much easier)
  ...
  if (extra_data && extra_length)
  {
    ...
    avi_write_data(avi_file->file, (char *)extra_data, extra_length);
    if (is_extra_align)
...
//write data
  ...
  avi_write_data(avi_file->file, (char *)data, length);
  if (is_align)
    write_char(avi_file->file, 0);
...
//read extra data
  ...
  if (junk_tag == GM_MAKE_FOURCC('J', 'U', 'N', 'K'))
  {
    ...
    if (extra_data)
    {
      ...
      avi_read_data(avi_file->file, (char *)extra_data, junk_size);
    }
    else
    {
...
//read data
  ...
  avi_read_data(avi_file->file, (char *)data, strdata_len);
//*length = strdata_len;
...
int GMAVIConditionIndexSearch(int handle, GMSearchIndex* out_value, int mode, int value1, int value2)
{
  ...
  if (mode == GMAVI_SEARCH_GivenTime)
  {
    ...
    while (offset < avi_file->movi_len)
    {
      ...
      {
        avi_read_data(avi_file->file, (char*) &extra_info, junk_size);
