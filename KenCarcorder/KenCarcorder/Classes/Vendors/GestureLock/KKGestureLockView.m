...
const static CGFloat kNodeDefaultWidth = 6;
const static CGFloat kNodeDefaultHeight = 6;
...
- (void)layoutSubviews{
    ...
    for (int i = 0; i < self.numberOfGestureNodes ; i++) {
        ...
        button.frame = CGRectMake(fabs((self.buttonSize.width + horizontalNodeMargin) * column), fabs((self.buttonSize.height + verticalNodeMargin) * row), self.buttonSize.width, self.buttonSize.height);
...
- (void)setSelectedGestureNodeImage:(UIImage *)selectedGestureNodeImage{
    if (_selectedGestureNodeImage != selectedGestureNodeImage) {
		...
		UIImage *jpegImage = [UIImage imageWithData:UIImageJPEGRepresentation(selectedGestureNodeImage, 1.0) scale:1.0];
        if (self.buttons != nil && [self.buttons count] > 0) {
            for (UIButton *button in self.buttons) {
                [button setImage:jpegImage forState:UIControlStateSelected];
            }
        }
    }
}
